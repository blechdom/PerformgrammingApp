...
- (id) initWithAddress:(NSString *)a	{
	...
	if (self = [super init])	{
		...
		//else
        address = [NSString stringWithFormat:@"%@",a];
        /// Previously, I added \" to this string above to fix maxmsp issue with extra quote... balancing quotes by adding one at the end - but it also effected incoming OSC messages, so I took it out.
        //address = [a retain];
		valueCount = 0;
		value = nil;
		valueArray = nil;
