...
@implementation OSCValue
 ...
 - (NSString *) description	{
	switch (type)	{
		...
		case OSCValFloat:
			return [NSString stringWithFormat:@"<OSCVa f %f",*(float *)value];
		case OSCValString:
...
- (id) initWithString:(NSString *)n	{
    ...
    NSAssert1(NO,@"\t\terr: %s - BAIL",__func__);
	return nil;
}
...
- (int) bufferLength	{
	...
	switch (type)	{
		...
		break;
        case OSCValString:
			//return ROUNDUP4([(__bridge NSString *)value length]);
            return ROUNDUP4((strlen([(__bridge NSString *)value UTF8String]) + 1));
		//	break;
			break;
		//case OSCValString:
		//	return ROUNDUP4([(NSString *)CFBridgingRelease(value) length]);
		//	break;
		case OSCValBool:
		case OSCValNil:
		case OSCValInfinity:
		...
		case OSCValBlob:
            ...
            return ROUNDUP4(4 + [(NSData *)CFBridgingRelease(value) length]);
			break;
	}
	return 0;
}
